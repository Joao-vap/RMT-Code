...
files = ['../Gaussian/b1N10.dat', '../Gaussian/b1N100.dat',
         ...
         '../Gaussian/b4N10.dat', '../Gaussian/tb4N100.dat',]
...
#   N**(1/6) * (lambda_max/sigma - 2*sqrt(N))
#   onde sigma = 1/sqrt(2*beta)

    # sigma = 1/math.sqrt(2*beta)
    # adata = [N**(1/6) * (i / sigma - 2*math.sqrt(N)) for i in data[i]]

    adata = [(i * 2 / math.sqrt(2*beta) - 2) * N**(2/3) for i in data[i]]
    # centralize 
    mean = np.median(adata)
    adata = [i - mean for i in adata]
...
axs[0, 1].plot(x, y, color='red', alpha=0.8,label='Semi-Círculo de Wigner')
axs[1, 1].plot(x, y, color='red',alpha=0.8, label='Semi-Círculo de Wigner')
axs[2, 1].plot(x, y, color='red', alpha=0.8,label='Semi-Círculo de Wigner')
...
axs[0,0].plot(x, density_GOE, color='blue', alpha=0.4, label='Modelo de Matriz')
axs[1,0].plot(x, density_GUE, color='blue', alpha=0.4, label='Modelo de Matriz')
axs[2,0].plot(x, density_GSE, color='blue', alpha=0.4, label='Modelo de Matriz')
